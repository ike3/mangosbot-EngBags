...
function EngInventory_OpenOrClose(opt)
	...
	if ( opt == "open" ) then
		if (EngInventory_Allow_OpenResort == 1) then
			EngInventory_resort_required = ENGINVENTORY_MANDATORY;
			EngInventory_Allow_OpenResort = 0;
		else
			EngBags_PrintDEBUG("Resort not needed, skipped.");
		end
	
		EngInventory_edit_mode = 0;
		EngInventory_HilightBag_Active = -10;	-- Reset Hilight of bag slots if active
...
function EngInventory_hook_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button)
	EngBags_PrintDEBUG("event: ToggleDropDownMenu()");

	EngInventory_savedhookfunctions["ToggleDropDownMenu"](level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button);
