...
EngBank_LOCALIZATION = {
	...
	["string_searches"] = {
		...
		{ -- french
			{ "FOOD", "Rend ([0-9.]+) points de vie en ([0-9.]] sec" }, 
			{ "DRINK", "Rend ([0-9.]+) points de mana en ([0-9.]] sec" }, 
			{ "HEALINGPOTION", "Rend ([0-9.]+) à ([0-9.]+) points de vie." }, 
			{ "MANAPOTION", "Rend ([0-9.]+) à ([0-9.]+) points de mana." }, 
			{ "PROJECTILE", "Projectile" }, 
			{ "JUJU", "Juju" }, 
			{ "BANDAGE", " Bandage" }, 
			...
			{ "SCHEMATIC", "Schéma:" }, 
			{ "FORMULA", "Formule:" }, 
			{ "MINIPET", "clic droit pour invoquer et renvoyer votre" }, 
			{ "ROGUE_POISON", "Crippling Poison [IV]*" }, 
			{ "ROGUE_POISON", "Deadly Poison [IV]*" }, 
			{ "ROGUE_POISON", "Instant Poison [IV]*" }, 
			{ "ROGUE_POISON", "Mind-numbing Poison [IV]*" }, 
			...
			{ "ROGUE_POWDER", "Blinding Powder" }, 
			{ "ROGUE_POWDER", "Flash Powder" }, 
			{ "QUESTITEMS", "Objet de quête" }, -- shredder operating manual & green hills of stranglethorn 
			{ "KEYS", " Key" }		
		}
...
ENGINVENTORY_LOCALIZATION = {
	...
	-- window elements
	["EngInventory_Button_Sorting_off"] = {
		"Sorting: OFF",
		"Sorting: OFF",
		"Sorting: OFF"
		},	
	["EngInventory_Button_Sorting_off_tooltip"] = {
		"Sorting engine is disabled. Not sorting anything!",
		"Sorting engine is disabled. Not sorting anything!",
		"Sorting engine is disabled. Not sorting anything!"
		},	
	["EngInventory_Button_Sorting_on"] = {
		"Sorting: ON",
		"Sorting: ON",
		"Sorting: ON"
		},	
	["EngInventory_Button_Sorting_on_tooltip"] = {
		"Sorting engine is enabled. Have fun :)",
		"Sorting engine is enabled. Have fun :)",
		"Sorting engine is enabled. Have fun :)"
		},	
...
function EngInventory_load_Localization(loadlang)
	...
	-- the string.gsub is there because the editor I'm using is being stupid and is randomly replaceing spaces with tabs.
	for key,value in pairs(ENGINVENTORY_LOCALIZATION) do
		if (type(value[loadlang_num]) == "string") then
			EngBags_PrintDEBUG("localization: "..key.." set to '"..value[loadlang_num].."'");
