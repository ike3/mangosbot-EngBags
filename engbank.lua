...
EngBank_HilightBag_Active = -10; -- Enable or disable hilight of one bag slots
EngBank_RightClickMenu_mode = "";
EngBank_RightClickMenu_opts = {};
EngBank_Bags = { -1, 5, 6, 7, 8, 9, 10, 11}
...
EngBank_Sorting_Active = 1; --For sorting enable/disable.
...
EngBank_ConfigOptions_Default = {
	...
	{	-- Show "Bag 11"
		...
		{ ["type"] = "Slider", ["ID"] = 1, ["width"] = 0.4, ["minValue"] = 0, ["maxValue"] = 1, ["valueStep"] = 1,
		  ["defaultValue"] = function()
				return EngBankConfig["show_Bag11"];
			end,
		  ["func"] = function(v)
				EngBankConfig["show_Bag11"] = tonumber(v);
				EngBank_SetDefaultValues(0);
			end
		},
		{ ["type"] = "Text", ["ID"] = 3, ["width"] = 0.1, ["color"] = { 0,1,0.5 }, ["align"] = "left", ["text"] = "On" }
...
function EngBank_SetDefaultValues(re)
	...
	EBank_SetDefault("show_Bag11", 1, 1+re, EngBags_NumericRange, 0, 1);
	...
	EBank_SetDefault("putinslot--CUSTOM_07", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_08", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_09", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_10", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_11", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_12", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_13", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_14", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EBank_SetDefault("putinslot--CUSTOM_15", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	--
...
function EngBank_Update_BagSlots()
	...
	for bagnum=5, 11 do
		local BankSlotLink = GetInventoryItemLink("player",ContainerIDToInventoryID(bagnum));
		if BankSlotLink ~= nil then
			...
			SetItemButtonTexture(getglobal("B4nkFrameBag"..(bagnum-4)), itemTexture);
		else
			SetItemButtonTexture(getglobal("B4nkFrameBag"..(bagnum-4)), "Interface\\PaperDoll\\UI-Backpack-EmptySlot.blp");
		end
	end
	...
	B4nkFrameBag7:SetScale(0.7);

	-- Dunno why hidding the empty slots ...
	...
	--if (numSlots < 7) then B4nkFrameBag7:Hide(); else B4nkFrameBag7:Show(); end
...
--		end
--
--		for key,value in pairs({ "HeadSlot","NeckSlot","ShoulderSlot","BackSlot","ChestSlot",
...
--				for a,b,c,d in string.gmatch(itemLink, "(%d+):(%d+):(%d+):(%d+)") do
--					itemLink = ""..a..":0:"..c..":0";
--				end
--				
			...
			if (bagNumSlots > 0) then
				...
				for slotnum = 1, bagNumSlots do
					...
					if (itm["itemlink"] ~= nil) then
						...
						itm["itemname"], itm["itemlink2"], itm["itemRarity"], _, itm["itemMinLevel"], itm["itemtype"], itm["itemsubtype"], itm["itemstackcount"], itm["itemloc"] = GetItemInfo(itm["itemlink"]);
...
function EngBank_UpdateButton(itemframe, itm)
	...
	if (EngBank_edit_mode == 1) then
        ...
        if ( EngBank_edit_mode == 1 ) then
				...
				-- Bag Hilight
				if (EngBank_HilightBag_Active > -10) then
					if (itm["bagnum"] ~= EngBank_HilightBag_Active) then
						itemframe_texture:SetVertexColor(1,1,1,0.15);
						itemframe_font:SetVertexColor(1,1,1,0.5);
...
function EngBank_ItemButton_OnClick(button, ignoreShift)
		...
		bar,position,itm = EngBank_GetBarPositionAndCache();		
...
function EngBank_Button_Sorting_OnClick()
	PlaySound("igMainMenuOptionCheckBoxOn");
	if (EngBank_Sorting_Active == 0) then
		EngBank_Sorting_Active = 1;
		EngBank_Button_Sorting:SetText(EILocal["EngInventory_Button_Sorting_on"]);
		EngBank_resort_required = EngBank_MANDATORY;
	else
		EngBank_Sorting_Active = 0;
		EngBank_Button_Sorting:SetText(EILocal["EngInventory_Button_Sorting_off"]);
	end
	EngBank_window_update_required = EngBank_MANDATORY;
	EngBank_UpdateWindow();
end

function EngBank_HilightBag(bagnum)
	PlaySound("igMainMenuOptionCheckBoxOn");
	if (bagnum == 0) then
		bagnum = -1;
	else
	...
	if (EngBank_HilightBag_Active == bagnum) then
		-- Already hilighted, remove hilight.
		EngBank_HilightBag_Active = -10;
	else
		-- Apply hilight to the bag slots
		EngBank_HilightBag_Active = bagnum;
	end
	EngBank_window_update_required = EngBank_MANDATORY;
	EngBank_UpdateWindow();	
end
...
function EngBank_SlotTargetButton_OnClick(button, ignoreShift)
        ...
        if (EngBank_edit_mode == 1) then
                for tmp in string.gmatch(this:GetName(), "EngBank_frame_SlotTarget_(%d+)") do
                        bar = tonumber(tmp);
                end
...
function EngBank_frame_RightClickMenu_populate(frame, level)
	local bar, position, bagnum, slotnum;
		...
		elseif (level == 3) then
			if ( this.value ~= nil ) then
				if ( this.value["opt"] == "override_placement_select" ) then
					for key,barclass in pairs(EngBank_Catagories(EngBank_Catagories_Exclude_List, this.value["select_bar"])) do
						info = {
							["text"] = barclass;
	...
	elseif (EngBank_RightClickMenu_mode == "mainwindow") then
		if (level == 1) then
			...
			info = {
				...
				["value"] = nil,
				["func"] = function()
						if (EngBankConfig["hide_hilightbag_buttons"] == 0) then
							EngBankConfig["hide_hilightbag_buttons"] = 1;
						else
							EngBankConfig["hide_hilightbag_buttons"] = 0;
						end
						EngBank_window_update_required = EngBank_MANDATORY;
						EngBank_UpdateWindow();
					end
			...
			info = {
				...
				["value"] = nil,
				["func"] = function()
						if (EngBankConfig["hide_sorting_button"] == 0) then
							EngBankConfig["hide_sorting_button"] = 1;
						else
							EngBankConfig["hide_sorting_button"] = 0;
						end
						EngBank_window_update_required = EngBank_MANDATORY;
						EngBank_UpdateWindow();
					end
			...
			info = {
				...
				["value"] = nil,
				["func"] = function()
						if (EngBankConfig["hide_bag_slots"] == 1) then
							EngBankConfig["hide_bag_slots"] = 0;
						else
							EngBankConfig["hide_bag_slots"] = 1;
						end
						EngBank_window_update_required = EngBank_MANDATORY;
						EngBank_UpdateWindow();
					end
	...
	local bagnum, slotnum;
        ...
        if (table.getn(EngBank_bar_positions[barnum]) > 0) then
			...
			slotnum = EngBank_bar_positions[barnum][position]["slotnum"];
                        ...
                        EngBank_MoveAndSizeFrame("EngBank_frame_Item_"..currentbutton.."_bkgr", "TOPLEFT",
                                "EngBank_frame_Item_"..currentbutton, "TOPLEFT",
...
function EngBank_UpdateWindow()
	...
	if (EngBank_Sorting_Active == 1) then
	...
	if (EngBank_window_update_required > EngBank_NOTNEEDED) then
		...
		if (currentbutton <= EngBank_MAXBUTTONS) then
			for currentbutton = currentbutton, EngBank_MAXBUTTONS do
				...
				tmpframe:Hide();
				tmpframe = getglobal("EngBank_frame_Item_"..(currentbutton).."_bkgr");
				tmpframe:Hide();
			end
		end
		...
		if (EngBankConfig["show_top_graphics"] == 1) then
			new_height = cur_y + 25;
			if (EngBankConfig["frameButtonSize"]==35) then
				EngBank_Button_Close:SetScale(0.9);
				EngBank_Button_MoveLockToggle:SetScale(0.9);
				EngBank_Button_ChangeEditMode:SetScale(0.9);
				EngBank_Button_Sorting:SetScale(0.9);
				EngBank_Button_HighlightToggle:SetScale(0.9);
				EngBank_Button_HilightBag0:SetScale(0.9);
				EngBank_Button_HilightBag1:SetScale(0.9);
				EngBank_Button_HilightBag2:SetScale(0.9);
				EngBank_Button_HilightBag3:SetScale(0.9);
				EngBank_Button_HilightBag4:SetScale(0.9);
				EngBank_Button_HilightBag5:SetScale(0.9);
				EngBank_Button_HilightBag6:SetScale(0.9);
				EngBank_Button_HilightBag7:SetScale(0.9);
			elseif (EngBankConfig["frameButtonSize"]==30) then
				EngBank_Button_Close:SetScale(0.8);
				EngBank_Button_MoveLockToggle:SetScale(0.8);
				EngBank_Button_ChangeEditMode:SetScale(0.8);
				EngBank_Button_Sorting:SetScale(0.8);
				EngBank_Button_HighlightToggle:SetScale(0.8);
				EngBank_Button_HilightBag0:SetScale(0.8);
				EngBank_Button_HilightBag1:SetScale(0.8);
				EngBank_Button_HilightBag2:SetScale(0.8);
				EngBank_Button_HilightBag3:SetScale(0.8);
				EngBank_Button_HilightBag4:SetScale(0.8);
				EngBank_Button_HilightBag5:SetScale(0.8);
				EngBank_Button_HilightBag6:SetScale(0.8);
				EngBank_Button_HilightBag7:SetScale(0.8);
			elseif (EngBankConfig["frameButtonSize"]==20) then
				EngBank_Button_Close:SetScale(0.7);
				EngBank_Button_MoveLockToggle:SetScale(0.7);
				EngBank_Button_ChangeEditMode:SetScale(0.7);
				EngBank_Button_Sorting:SetScale(0.7);
				EngBank_Button_HighlightToggle:SetScale(0.7);
				EngBank_Button_HilightBag0:SetScale(0.7);
				EngBank_Button_HilightBag1:SetScale(0.7);
				EngBank_Button_HilightBag2:SetScale(0.7);
				EngBank_Button_HilightBag3:SetScale(0.7);
				EngBank_Button_HilightBag4:SetScale(0.7);
				EngBank_Button_HilightBag5:SetScale(0.7);
				EngBank_Button_HilightBag6:SetScale(0.7);
				EngBank_Button_HilightBag7:SetScale(0.7);
			else
				EngBank_Button_Close:SetScale(1);
				EngBank_Button_MoveLockToggle:SetScale(1);
				EngBank_Button_ChangeEditMode:SetScale(1);
				EngBank_Button_Sorting:SetScale(1);
				EngBank_Button_HighlightToggle:SetScale(1);
				EngBank_Button_HilightBag0:SetScale(1);
				EngBank_Button_HilightBag1:SetScale(1);
				EngBank_Button_HilightBag2:SetScale(1);
				EngBank_Button_HilightBag3:SetScale(1);
				EngBank_Button_HilightBag4:SetScale(1);
				EngBank_Button_HilightBag5:SetScale(1);
				EngBank_Button_HilightBag6:SetScale(1);
				EngBank_Button_HilightBag7:SetScale(1);
			end
		else
			new_height = cur_y + 25;
			EngBank_Button_Close:SetScale(1);
		...
		if (EngBank_Sorting_Active == 0) then
			frame:SetBackdropColor(0.7,0.7,0.7,EngBankConfig["bar_colors_"..EngBags_MAINWINDOWCOLORIDX.."_background_a"]);
			frame:SetBackdropBorderColor(1,1,1,EngBankConfig["bar_colors_"..EngBags_MAINWINDOWCOLORIDX.."_background_a"]);
		end
		
        EngBank_MoneyFrame.moneyType = "PLAYER"
		...
		if (EngBankConfig["show_top_graphics"] == 1) then
			...
			if (EngBankConfig["hide_sorting_button"] == 0) then
				EngBank_Button_Sorting:Show();
			else
				EngBank_Button_Sorting:Hide();
			end
			EngBank_Button_HighlightToggle:Show();
			if (EngBankConfig["hide_hilightbag_buttons"] == 0) then
				EngBank_Button_HilightBag0:Show();
				EngBank_Button_HilightBag1:Show();
				EngBank_Button_HilightBag2:Show();
				EngBank_Button_HilightBag3:Show();
				EngBank_Button_HilightBag4:Show();
				EngBank_Button_HilightBag5:Show();
				EngBank_Button_HilightBag6:Show();
				EngBank_Button_HilightBag7:Show();
			else
				EngBank_Button_HilightBag0:Hide();
				EngBank_Button_HilightBag1:Hide();
				EngBank_Button_HilightBag2:Hide();
				EngBank_Button_HilightBag3:Hide();
				EngBank_Button_HilightBag4:Hide();			
				EngBank_Button_HilightBag5:Hide();			
				EngBank_Button_HilightBag6:Hide();			
				EngBank_Button_HilightBag7:Hide();			
			end
			
			if (EngBankConfig["hide_bag_slots"] == 0) then
				B4nkFrameBag1:Show();
				B4nkFrameBag2:Show();
				B4nkFrameBag3:Show();
				B4nkFrameBag4:Show();
