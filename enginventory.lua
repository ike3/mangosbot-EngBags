...
EngInventory_HilightBag_Active = -10; -- Enable or disable hilight of one bag slots
EngInventory_Allow_OpenResort = 1; -- For performance.
EngInventory_resort_required = ENGINVENTORY_MANDATORY;
EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
EngInventory_Sorting_Active = 1; --For sorting enable/disable.
...
-- NOTE: From 2.0 this is not functional.
...
function EngInventory_CreateConfigOptions()
	...
	for key,value in ipairs(EngInventoryConfig["item_search_list"]) do
...
function EngInventory_Catagories(exclude_list, select_bar)
	...
	for key,value in pairs(EngInventoryConfig) do
...
function EngInventory_SetDefaultValues(re)
	...
	EI_SetDefault("putinslot--CLASS_ITEMS1", 15, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
        ...
        EI_SetDefault("putinslot--MINING_EMPTY_SLOTS", 13, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);         -- Empty slots go in this bar
	...
	EI_SetDefault("putinslot--CUSTOM_07", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_08", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_09", 10, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_10", 7, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_11", 7, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_12", 7, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_13", 7, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_14", 7, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	EI_SetDefault("putinslot--CUSTOM_15", 7, 1+re, EngBags_NumericRange, 1, EngBags_MAX_BARS);
	--
	...
	if (EngInventoryConfig["itemoverride_loaddefaults"] == 1) then
		...
		--for key,value in pairs(EILocal["string_searches"]) do
		--	if (value ~= nil) then
...
function EngInventory_SetTradeSkills()
	...
	for k,v in pairs(EngBagsConfig[EngBags_PLAYERID]["tradeskills"]) do
...
function EngInventory_init()
	...
	EngInventory_SetTradeSkills();
        ...
        EngInventory_Button_Sorting:SetText(EILocal["EngInventory_Button_Sorting_on"]);
...
function EngInventory_OnEvent(event)
	if ( event == "BAG_UPDATE" ) then
		EngBags_PrintDEBUG("OpenResort Allowed.");
		EngInventory_Allow_OpenResort = 1;
	end
	if ( EngInventory_frame:IsVisible() ) then
		EngBags_PrintDEBUG("EI Event Start: '"..event.."'");
        if ( event == "BAG_UPDATE" ) then
                EngInventory_UpdateWindow();
		...
		EngBags_PrintDEBUG("EI Event End: '"..event.."'");
	end
end
	...
	for index, bagnum in ipairs(EngInventory_Bags) do
		if (EngInventoryConfig["show_Bag"..bagnum] == 1) then
			...
			if (bagnum == KEYRING_CONTAINER) then
				if (EngInventoryConfig["hide_keyring_keys"] == 1) then
					bagNumSlots = 0;
				else
			...
			if (bagNumSlots > 0) then
				...
				for slotnum = 1, bagNumSlots do
					...
					if (itm["itemlink"] ~= nil) then
						EngBags_PrintDEBUG("Item Found:");
						-- there's an item in the bag, let's find out more about it
						EngBags_PrintDEBUG("Before: " .. itm["itemlink"]);
						for a,b,c,d in string.gmatch(itm["itemlink"], "(%d+):(%d+):(%d+):(%d+)") do
							if (a ~= "0") then 
							itm["itemlink"] = "item:"..a..":"..b..":"..c..":"..d;
							EngBags_PrintDEBUG("After: " .. itm["itemlink"]);
						...
						itm["itemname"], itm["itemlink2"], itm["itemRarity"], _, itm["itemMinLevel"], itm["itemtype"], itm["itemsubtype"], itm["itemstackcount"], _ = GetItemInfo(itm["itemlink"]);
						...
						if bagnum > 0 then
							local InvSlotLink = GetInventoryItemLink("player",ContainerIDToInventoryID(bagnum));
							if InvSlotLink ~= nil then
								local _, _, itemCode = strfind(InvSlotLink, "(%d+):");
								local _, _, _, _, _, _, itemSubtype, _, _, itemTexture = GetItemInfo(itemCode);
								...
								if (itemSubtype == "Soul Bag") then
									if (EngInventoryConfig["hide_bag_icons_profession"] == 1) then
										itm["texture"] = nil;
									end
								...
								if (itemSubtype == "Mining Bag") then
									if (EngInventoryConfig["hide_bag_icons_profession"] == 1) then
										itm["texture"] = nil;
									end
									itm["keywords"]["MINING_BAG"]=1;
								end
								...
								if (itemSubtype == "Quiver") then
									if (EngInventoryConfig["hide_bag_icons_quiver"] == 1) then
										itm["texture"] = nil;
									end
								...
								if (EngInventoryConfig["hide_bag_icons_normal"]==1 and itm["keywords"]["AMMO_BAG"]~=1 and itm["keywords"]["ARROW_BAG"]~=1 and itm["keywords"]["HERB_BAG"]~=1 and itm["keywords"]["ENCHANT_BAG"]~=1 and itm["keywords"]["MINING_BAG"]~=1 and itm["keywords"]["SOUL_BAG"]~=1) then
									itm["texture"] = nil;
								end
							end
						else
...
function EngInventory_PickBar(itm)
        if (itm["itemlink"] == nil) then
            ...
            elseif (itm["keywords"]["ENCHANT_BAG"]) then
				...
				return itm;
            elseif (itm["keywords"]["MINING_BAG"]) then
				itm["bar"] = EngInventoryConfig["putinslot--MINING_EMPTY_SLOTS"];
				while (type(itm["bar"]) ~= "number") do
					itm["bar"] = EngInventoryConfig[itm["bar"]];
				end
				itm["barClass"] = "MINING_EMPTY_SLOTS";
				return itm;
			else
...
function EngInventory_UpdateButton(itemframe, itm)
	...
	if (EngInventory_edit_mode == 1) then
        ...
        if ( EngInventory_edit_mode == 1 ) then
				...
				-- Bag Hilight
				if (EngInventory_HilightBag_Active > -10) then
					if (itm["bagnum"] ~= EngInventory_HilightBag_Active) then
						itemframe_texture:SetVertexColor(1,1,1,0.15);
						itemframe_font:SetVertexColor(1,1,1,0.5);
						itemframe_bkgr:SetVertexColor(1,1,1,0.5);
					end
...
function EngInventory_ItemButton_OnEnter()
	...
	EngBags_PrintDEBUG("Rollover Info | Bagnum::"..this:GetParent():GetID().." - Slotnum::"..this:GetID());
...
function EngInventory_Button_Sorting_OnClick()
	PlaySound("igMainMenuOptionCheckBoxOn");
	if (EngInventory_Sorting_Active == 0) then
		EngInventory_Sorting_Active = 1;
		EngInventory_Button_Sorting:SetText(EILocal["EngInventory_Button_Sorting_on"]);
		EngInventory_resort_required = ENGINVENTORY_MANDATORY;
	else
		EngInventory_Sorting_Active = 0;
		EngInventory_Button_Sorting:SetText(EILocal["EngInventory_Button_Sorting_off"]);
	end
	EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
	EngInventory_UpdateWindow();
end

function EngInventory_HilightBag(bagnum)
	PlaySound("igMainMenuOptionCheckBoxOn");
	if (EngInventory_HilightBag_Active == bagnum) then
		-- Already hilighted, remove hilight.
		EngInventory_HilightBag_Active = -10;
	else
		-- Apply hilight to the bag slots
		EngInventory_HilightBag_Active = bagnum;
	end
	EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
	EngInventory_UpdateWindow();	
end
...
EI_Orig_ContainerFrameItemButton_OnClick = ContainerFrameItemButton_OnClick;
...
function EngInventory_Button_ChangeEditMode_OnClick()
	...
	if (EngInventory_edit_mode == 0) then
		...
		-- ContainerFrameItemButton_OnClick = EngInventory_ItemButton_OnClick;
	else
		EngInventory_edit_mode = 0;
		EngInventory_Button_ChangeEditMode:SetText(EILocal["EngInventory_Button_ChangeEditMode_off"]);
		-- ContainerFrameItemButton_OnClick = EI_Orig_ContainerFrameItemButton_OnClick;
	end
	EngInventory_resort_required = ENGINVENTORY_MANDATORY;
...
function EngInventory_SlotTargetButton_OnClick(button, ignoreShift)
        ...
        if (EngInventory_edit_mode == 1) then
                for tmp in string.gmatch(this:GetName(), "EngInventory_frame_SlotTarget_(%d+)") do
                        bar = tonumber(tmp);
                end
...
function EngInventory_frame_RightClickMenu_populate(frame, level)
	local bar, position, bagnum, slotnum;
	...
	elseif (EngInventory_RightClickMenu_mode == "mainwindow") then
		if (level == 1) then
			...
			info = {
				...
				["value"] = nil,
				["func"] = EngInventory_Button_Sorting_OnClick
				};
			if (EngInventory_Sorting_Active == 0) then
				info["checked"] = 1;
			end
			...
			info = {
				...
				["value"] = nil,
				["func"] = function()
						if (EngInventoryConfig["hide_hilightbag_buttons"] == 0) then
							EngInventoryConfig["hide_hilightbag_buttons"] = 1;
						else
							EngInventoryConfig["hide_hilightbag_buttons"] = 0;
						end
						EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
						EngInventory_UpdateWindow();
					end
			...
			info = {
				...
				["value"] = nil,
				["func"] = function()
						if (EngInventoryConfig["hide_sorting_button"] == 0) then
							EngInventoryConfig["hide_sorting_button"] = 1;
						else
							EngInventoryConfig["hide_sorting_button"] = 0;
						end
						EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
						EngInventory_UpdateWindow();
					end
			...
			info = {
				["text"] = "Show Search Frame",
				["value"] = nil,
				["func"] = function()
						if (EngInventoryConfig["hide_search_frame"] == 0) then
							EngInventoryConfig["hide_search_frame"] = 1;
						else
							EngInventoryConfig["hide_search_frame"] = 0;
						end
						EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
						EngInventory_UpdateWindow();
					end
			...
			info = {
				["text"] = "Hide Keyring Keys",
				["value"] = nil,
				["func"] = function()
						if (EngInventoryConfig["hide_keyring_keys"] == 0) then
							EngInventoryConfig["hide_keyring_keys"] = 1;
						else
							EngInventoryConfig["hide_keyring_keys"] = 0;
						end
						-- Update the cache and resort items to clean up the icons correctly.
						EngInventory_Update_item_cache();
						EngInventory_Sort_item_cache();
						EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
						EngInventory_UpdateWindow();
			...
			info = {
				["text"] = "Copy Inv. Settings to Bank";
				["value"] = nil,
				["func"] = function()
						EngBagsConfig["Bank"][EngBags_PLAYERID]["item_search_list"] = EngBags_copyTable(EngBagsConfig["Inventory"][EngBags_PLAYERID]["item_search_list"]);
						EngBagsConfig["Bank"][EngBags_PLAYERID]["item_overrides"] = EngBags_copyTable(EngBagsConfig["Inventory"][EngBags_PLAYERID]["item_overrides"]);
						for key,value in ipairs(EngInventoryConfig["item_search_list"]) do
							if (EngInventoryConfig["putinslot--"..value[1]] ~= nil) then
								EngBagsConfig["Bank"][EngBags_PLAYERID]["putinslot--"..value[1]] = EngInventoryConfig["putinslot--"..value[1]];
							end
						end
						EngBank_SetDefaultValues(0);
						EngBank_window_update_required = ENGINVENTORY_MANDATORY;
						EngBank_UpdateWindow();
						EngBags_Print("Settings from inventory copied to bank. If you want to cancel this, use control-alt-del and kill the wow.exe process. (All changes in this session will be lost)");
					end
				};
			UIDropDownMenu_AddButton(info, level);
		...
		elseif (level == 2) then
			if (this.value ~= nil) then
				...
				if (this.value["opt"] == "import_config") then
					for key,value in pairs(EngBagsItems) do
				...
				if (this.value["opt"] == "set_icon_hide") then
					info = {
						["text"] = "Hide All",
						["value"] = nil,
						["func"] = function()
								if (EngInventoryConfig["hide_bag_icons"] == 1) then
									...
									EngInventoryConfig["hide_bag_icons_normal"] = 0;
									EngInventoryConfig["hide_bag_icons_profession"] = 0;
									EngInventoryConfig["hide_bag_icons_quiver"] = 0;
								end
								EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
								EngInventory_UpdateWindow();
							end
					...
					info = {
						["text"] = "Hide Normal Bags",
						["value"] = nil,
						["func"] = function()
								if (EngInventoryConfig["hide_bag_icons_normal"] == 1) then
									EngInventoryConfig["hide_bag_icons_normal"] = 0;
								else
									EngInventoryConfig["hide_bag_icons_normal"] = 1;
									EngInventoryConfig["hide_bag_icons"] = 0;
								end
					...
					info = {
						["text"] = "Hide Profession Bags",
						["value"] = nil,
						["func"] = function()
								if (EngInventoryConfig["hide_bag_icons_profession"] == 1) then
									EngInventoryConfig["hide_bag_icons_profession"] = 0;
								else
									EngInventoryConfig["hide_bag_icons_profession"] = 1;
									EngInventoryConfig["hide_bag_icons"] = 0;
								end
								EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
								EngInventory_UpdateWindow();
					...
					info = {
						["text"] = "Hide Quiver/Ammo Bags",
						["value"] = nil,
						["func"] = function()
								if (EngInventoryConfig["hide_bag_icons_quiver"] == 1) then
									EngInventoryConfig["hide_bag_icons_quiver"] = 0;
								else
									EngInventoryConfig["hide_bag_icons_quiver"] = 1;
									EngInventoryConfig["hide_bag_icons"] = 0;
								end
								EngInventory_window_update_required = ENGINVENTORY_MANDATORY;
								EngInventory_UpdateWindow();
...
function EngInventory_UpdateWindow()
	...
	if (EngInventory_Sorting_Active == 1) then
	...
	if (EngInventory_window_update_required > ENGINVENTORY_NOTNEEDED) then
		...
		if (EngInventoryConfig["hide_search_frame"] == 1) then
			EngInventory_SearchFrame:Hide();
			EngInventory_frame_searchtitle:Hide();
		else
			EngInventory_SearchFrame:Show();		
			EngInventory_frame_searchtitle:Show();
		end
		...
		if (EngInventoryConfig["show_top_graphics"] == 1) then
			...
			if (EngInventoryConfig["frameButtonSize"]==35) then
				...
				EngInventory_Button_Sorting:SetScale(0.9);
				EngInventory_Button_HighlightToggle:SetScale(0.9);
				EngInventory_Button_HilightBag0:SetScale(0.9);
				EngInventory_Button_HilightBag1:SetScale(0.9);
				EngInventory_Button_HilightBag2:SetScale(0.9);
				EngInventory_Button_HilightBag3:SetScale(0.9);
				EngInventory_Button_HilightBag4:SetScale(0.9);
			elseif (EngInventoryConfig["frameButtonSize"]==30) then
				...
				EngInventory_Button_Sorting:SetScale(0.8);
				EngInventory_Button_HighlightToggle:SetScale(0.8);
				EngInventory_Button_HilightBag0:SetScale(0.8);
				EngInventory_Button_HilightBag1:SetScale(0.8);
				EngInventory_Button_HilightBag2:SetScale(0.8);
				EngInventory_Button_HilightBag3:SetScale(0.8);
				EngInventory_Button_HilightBag4:SetScale(0.8);
			elseif (EngInventoryConfig["frameButtonSize"]==20) then
				...
				EngInventory_Button_Sorting:SetScale(0.7);
				EngInventory_Button_HighlightToggle:SetScale(0.7);
				EngInventory_Button_HilightBag0:SetScale(0.7);
				EngInventory_Button_HilightBag1:SetScale(0.7);
				EngInventory_Button_HilightBag2:SetScale(0.7);
				EngInventory_Button_HilightBag3:SetScale(0.7);
				EngInventory_Button_HilightBag4:SetScale(0.7);
			else
				...
				EngInventory_Button_Sorting:SetScale(1);
				EngInventory_Button_HighlightToggle:SetScale(1);
				EngInventory_Button_HilightBag0:SetScale(1);
				EngInventory_Button_HilightBag1:SetScale(1);
				EngInventory_Button_HilightBag2:SetScale(1);
				EngInventory_Button_HilightBag3:SetScale(1);
				EngInventory_Button_HilightBag4:SetScale(1);
			end
		else
			new_height = cur_y + 25;
			EngInventory_Button_Close:SetScale(1);
...
--		frame:SetScale(EngBagsConfig["frameWindowScale"]);
		...
		if (EngInventory_Sorting_Active == 0) then
			frame:SetBackdropColor(0.7,0.7,0.7,EngInventoryConfig["bar_colors_"..EngBags_MAINWINDOWCOLORIDX.."_background_a"]);
			frame:SetBackdropBorderColor(1,1,1,EngInventoryConfig["bar_colors_"..EngBags_MAINWINDOWCOLORIDX.."_background_a"]);
		end
		...
		if (EngInventoryConfig["show_top_graphics"] == 1) then
			...
			if (EngInventoryConfig["hide_sorting_button"] == 0) then
				EngInventory_Button_Sorting:Show();
			else
				EngInventory_Button_Sorting:Hide();
			end
			EngInventory_Button_HighlightToggle:Show();
			if (EngInventoryConfig["hide_hilightbag_buttons"] == 0) then
				EngInventory_Button_HilightBag0:Show();
				EngInventory_Button_HilightBag1:Show();
				EngInventory_Button_HilightBag2:Show();
				EngInventory_Button_HilightBag3:Show();
				EngInventory_Button_HilightBag4:Show();
			else
				EngInventory_Button_HilightBag0:Hide();
				EngInventory_Button_HilightBag1:Hide();
				EngInventory_Button_HilightBag2:Hide();
				EngInventory_Button_HilightBag3:Hide();
				EngInventory_Button_HilightBag4:Hide();			
			end
