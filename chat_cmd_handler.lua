...
function EngBank_cmd(msg)
	...
	elseif (cmd == "disable") then
		...
		EngBags_Print("EngBank disabled.");
	elseif (cmd == "enable") then
		EngReplaceBank = 1;
		EngBank_SetReplaceBank();
		EngBags_Print("EngBank enabled.");
	...
	elseif (cmd == "test") then
		...
		for key,value in pairs(EngBank_HELP[ EILocal["help_text"] ]) do
...
function EngInventory_cmd(msg)
	...
	elseif (cmd == "test") then
		...
		for key,value in pairs(ENGINVENTORY_HELP[ EILocal["help_text"] ]) do
			EngBags_Print(string.gsub(value, "\r", ""));
		end
	end
