...
if (FluidFrames ~= nil) then
	EngBags_FF_Hook_Old = FluidFrames.InitTempDraggableFrames;
	EngBags_FF_Hook = function()
		if (EngBags_FF_Hook_Old ~= nil) then
			EngBags_FF_Hook_Old();
			local titleRegion = EngInventory_frame:GetTitleRegion();
			if (titleRegion ~= nil) then
				titleRegion:SetPoint("BOTTOMLEFT", "EngInventory_frame", "BOTTOMLEFT", 0, 0);
				titleRegion:SetPoint("TOPLEFT", "EngInventory_frame", "BOTTOMLEFT", 0, 0);
				titleRegion:SetPoint("BOTTOMRIGHT", "EngInventory_frame", "BOTTOMLEFT", 0, 0);
				titleRegion:SetPoint("TOPRIGHT", "EngInventory_frame", "BOTTOMLEFT", 0, 0);
			end
			local titleRegion = EngBank_frame:GetTitleRegion();
			if (titleRegion ~= nil) then
				titleRegion:SetPoint("BOTTOMLEFT", "EngBank_frame", "BOTTOMLEFT", 0, 0);
				titleRegion:SetPoint("TOPLEFT", "EngBank_frame", "BOTTOMLEFT", 0, 0);
				titleRegion:SetPoint("BOTTOMRIGHT", "EngBank_frame", "BOTTOMLEFT", 0, 0);
				titleRegion:SetPoint("TOPRIGHT", "EngBank_frame", "BOTTOMLEFT", 0, 0);
			end
		end
	end
	FluidFrames.InitTempDraggableFrames = EngBags_FF_Hook;
end
...
function EngBank_hook_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button)
	EngBags_PrintDEBUG("event: ToggleDropDownMenu()");

	EngBank_savedhookfunctions["ToggleDropDownMenu"](level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button);
